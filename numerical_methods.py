...
def _crear_evaluador_frac(expresion: str) -> Callable[[Fraction], Fraction]:
    ...
    """
    # Reemplaza el operador ^ (común en calculadoras) por ** (de Python)
    ...
    def evaluador(x_val: Fraction) -> Fraction:
        ...
        """
        try:
            # Preparamos el contexto local para esta evaluación
...
def _crear_evaluador_numpy(expresion: str) -> Callable[[np.ndarray], np.ndarray]:
    ...
    def evaluador(x_val: np.ndarray) -> np.ndarray:
        """
        La función interna que se llamará con un arreglo de x.
        """
        try:
            contexto_local = contexto_seguro.copy()
            contexto_local['x'] = x_val
            ...
            # Asegurar que la función devuelva un arreglo si el usuario solo puso un número (ej. f(x) = 5)
            ...
            # Captura errores (ej. "log(-1)") y los silencia para el gráfico
